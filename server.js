...
async function regenerateBulkCache() {
  ...
  try {
    const [players, sessions, servers, mmr, sr, records, stats, bannedGuids, totalCounts, avatars] = await Promise.race([
      Promise.all([
        ...
        db.getTotalCounts(),
        // ALL player avatars (lightweight - just GUID:URL map)
        db.getAllPlayerAvatars()
      ]),
      timeout
    ]);
    // Merge real counts into stats
    const fullStats = { ...stats, ...totalCounts };
    bulkResponseCache = { data: { players, sessions, servers, leaderboards: { mmr, sr }, records, stats: fullStats, bannedGuids, avatars }, timestamp: Date.now(), generating: false };
...
// Polls
app.get('/api/polls', async (req, res) => { try { res.json(await db.getActivePolls()); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/polls/expired', async (req, res) => { try { res.json(await db.getExpiredPolls()); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/admin/polls', requireAuth, requireModerator, async (req, res) => { try { res.json(await db.getAllPolls()); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/polls', requireAuth, requireAdmin, async (req, res) => { try { const { question, options, expiresAt, showResults } = req.body; if (!question || !options || options.length < 2) return res.status(400).json({ error: 'Question and at least 2 options required' }); res.json({ success: true, poll: await db.createPoll({ question, options, createdBy: req.userId, createdByName: req.userProfile?.displayName || 'Admin', expiresAt, showResults }) }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/polls/:id/vote', requireAuth, async (req, res) => { try { const { optionIndex } = req.body; if (optionIndex === undefined || optionIndex < 0) return res.status(400).json({ error: 'Option index required' }); const poll = await db.votePoll(req.params.id, req.userId, optionIndex); poll ? res.json({ success: true, poll }) : res.status(404).json({ error: 'Poll not found or already voted' }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/polls/:id/toggle', requireAuth, requireAdmin, async (req, res) => { try { const p = await db.togglePollActive(req.params.id); p ? res.json({ success: true, poll: p }) : res.status(404).json({ error: 'Not found' }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/polls/:id', requireAuth, requireAdmin, async (req, res) => { try { const d = await db.deletePoll(req.params.id); d ? res.json({ success: true }) : res.status(404).json({ error: 'Not found' }); } catch (err) { res.status(500).json({ error: err.message }); } });
...
app.get('/api/admin/streamer-applications', requireAuth, requireModerator, async (req, res) => { try { res.json(await db.getAllStreamerApplications(req.query.status || null)); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/streamer-applications/:id/claim', requireAuth, requireModerator, async (req, res) => { try { const a = await db.claimStreamerApplication(req.params.id, req.userProfile?.displayName || req.user?.email, req.userProfile?.linkedPlayerGuid); a ? res.json({ success: true, application: a }) : res.status(404).json({ error: 'Not found or already claimed' }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/streamer-applications/:id/resolve', requireAuth, requireModerator, async (req, res) => { try { const { approved, resolution } = req.body; const a = await db.resolveStreamerApplication(req.params.id, req.userProfile?.displayName || req.user?.email, req.userProfile?.linkedPlayerGuid, approved, resolution || ''); a ? res.json({ success: true, application: a }) : res.status(404).json({ error: 'Not found' }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/streamer-applications', requireAuth, async (req, res) => { try { const { playerName, channelName, channelUrl, additionalInfo } = req.body; if (!playerName || !channelName || !channelUrl) return res.status(400).json({ error: 'Name, channel name and URL required' }); const hasActive = await db.hasActiveStreamerApplication(req.userId); if (hasActive) return res.status(400).json({ error: 'You already have a pending application' }); res.json({ application: await db.createStreamerApplication({ userId: req.userId, playerGuid: req.userProfile?.linkedPlayerGuid, playerName, channelName, channelUrl, additionalInfo }) }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/streamer-applications/my', requireAuth, async (req, res) => { try { res.json(await db.getUserStreamerApplications(req.userId)); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/streamer-applications/status', requireAuth, async (req, res) => { try { const apps = await db.getUserStreamerApplications(req.userId); const approved = apps.find(a => a.status === 'approved'); const pending = apps.find(a => a.status === 'pending' || a.status === 'claimed'); res.json({ isStreamer: !!approved, hasPendingApplication: !!pending, applications: apps }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/streamer-applications/revoke-my-status', requireAuth, async (req, res) => { try { const deletedCount = await db.deleteAllStreamerApplicationsByUserId(req.userId); res.json({ success: true, deletedApplications: deletedCount, message: `Revoked streamer status - deleted ${deletedCount} applications` }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/streamer-applications/:id', requireAuth, requireAdmin, async (req, res) => { try { const deleted = await db.deleteStreamerApplication(req.params.id); deleted ? res.json({ success: true }) : res.status(404).json({ error: 'Not found' }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/streamers/:guid/revoke', requireAuth, requireAdmin, async (req, res) => { try { const guid = req.params.guid.toUpperCase(); const deletedCount = await db.deleteAllStreamerApplicationsByGuid(guid); res.json({ success: true, deletedApplications: deletedCount, message: `Revoked streamer status and deleted ${deletedCount} applications for ${guid}` }); } catch (err) { res.status(500).json({ error: err.message }); } });
